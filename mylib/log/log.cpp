...
MYLIB_SPACE_BEGIN
LogEvent::LogEvent(String loggerName, String file, String funcName, String className, String threadName, String coroutineName, Type type, uint32 line, time_t t, tid_t thread, cid_t coroutine)
...
void ConsoleAppender::append(const LogEvent &event) {
  ...
  Lock::Ground g{m_lock};
  std::cout << m_formatter->format(event);
}

ConsoleAppender::ConsoleAppender(LogEvent::Type type, LogFormatter::ptr formatter)
...
FileAppender::FileAppender(String file, LogEvent::Type type, LogFormatter::ptr formatter) : LogAppender(type, std::move(formatter)), m_file_path(std::move(file)) {}
...
public:
  ...
  [[nodiscard]] String format(const LogEvent &event) const override { return mylib::StringConverter<mylib::LogEvent::Type>::toString(event.getType()); }
};
...
public:
  ...
  [[nodiscard]] String format(const LogEvent &event) const override {
    char tmp[64];
    strftime(tmp, sizeof(tmp), m_extend.c_str(), localtime(&event.getTime()));
    return tmp;
  }
};
...
LogWarp::LogWarp(LogEvent::Type level, Logger::ptr logger, String file, String funcName, String className, uint32 line)
    : m_level(level),
